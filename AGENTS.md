...
- Check `status: pending-approval | approved` in a doc's frontmatter before treating it as ready for the next pipeline stage to consume — and if it is already `approved` with no changed upstream input, that is your signal to **skip re-reading/re-verifying it**, not a formality to note in passing. Follow `<reusablesPath>/docs/standards/agent-efficiency-standards.md` for the full incremental-work/caching *and* session/fan-out discipline (don't rescan what's already done; read once per task, not once per reference; resume interrupted work from the last valid checkpoint instead of restarting; keep sessions short and compacted, and bound how many agents/services get spawned or swept in one pass instead of running the whole pipeline or the whole service backlog "since we're here").
- Trust `README.md`'s Agent Pipeline status board for what's done per service instead of rediscovering it by opening every service's docs — that's exactly the redundant rescanning the efficiency standard above tells you to avoid.
