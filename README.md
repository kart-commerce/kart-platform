...
All 17 remaining BRD services have a drafted `requirement-spec.md` + `edge-cases.md` in `docs/services/<name>/`. A professional-grade review pass has since run against all of them (plus `kart-offer-service`'s still-pending `edge-cases.md`): **9 passed and are now `status: approved`** (User, Category, Search, Cart, Payment, Notification, Shipping, Recommendation, Analytics) — clear to start `architecture-agent`. **9 have concrete, named gaps and remain `pending-approval`** (Identity, Product, Inventory, Order, Wishlist, Review, Delivery Tracking, Admin, and Offer's `edge-cases.md`) — see [`docs/services/README.md`](docs/services/README.md) for the full index and the per-service gap table, plus the running list of cross-service BRD contradictions surfaced while drafting and reviewing (e.g. Order↔Shipping integration direction, Notification's and Analytics' actual consumed-event sets, several published events missing from the Event Catalog, and two newly found missing-publisher gaps for `OrderDelivered` and `InventoryReleased`).
