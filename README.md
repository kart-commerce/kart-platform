...
All 17 remaining BRD services now have a drafted `requirement-spec.md` + `edge-cases.md` in `docs/services/<name>/`, `status: pending-approval` — see [`docs/services/README.md`](docs/services/README.md) for the full index, including a list of cross-service BRD contradictions surfaced while drafting (e.g. Order↔Shipping integration direction, Notification's and Analytics' actual consumed-event sets, several published events missing from the Event Catalog). None of these are approved yet — review them before any architecture-agent work starts on a new service.
