...
Per BRD §24.1.4, the service whose database physically holds a row decides and enforces that row's row-level security. Search has no PostgreSQL write model at all (see "Architecture Note" above), so native RLS's `ENABLE ROW LEVEL SECURITY`/`CREATE POLICY` mechanism has no write-side table to attach to in the first place. Its store is OpenSearch — not MongoDB either, but the same underlying question §24.1.4 poses for a MongoDB read model applies equally here: does any collection/index need a per-caller ownership filter pushed into a shared, unbypassable query-builder at the data-access boundary?

- **`search_products`, `search_category_lookup`, `search_rating_ledger`** — none carry a per-row end-user ownership column. Every document is public storefront/search content (a SKU's searchable attributes, a category's display name, a SKU's aggregate rating) with no analogue to Order/Cart/Wishlist's `user_id`-keyed rows — the same no-ownership-dimension carve-out already established for `kart-product-service`'s and `kart-category-service`'s own catalog tables (BRD §24.1.4's own Payment `payment_intents` precedent: a row-level policy needs an ownership column to key on, and none exists here). The one query-time filter this service does apply — the implicit `availability: Active` term filter ("Default Query Filter" above) — is a **business-visibility rule** (soft-removed items excluded from default results), not an access-control/ownership predicate, and is already centralized at the single `/search` query path rather than left to per-call-site discipline, which is the structural property §24.1.4's Mongo-alternative mechanism asks for even though the underlying reason here is different (result curation, not caller-scoped ownership).
- No principal-scoped read filter is needed for any of the three indices — every caller, authenticated or not, is entitled to see the same searchable catalog (§24.1.2 above: `CanRead` is unconditional).

## Sensitive / PII Column Classification (BRD §24.1.5)

**No sensitive/PII columns exist.** Every field across `search_products` (`name`, `description`, `brand`, `category`, `price`, `availability`, `size`/`color`/`extendedAttributesRaw`, `sponsored`, `rating`) and `search_category_lookup` (`categoryId`, `categoryName`) is public storefront/search content, not personal data about any individual. `search_rating_ledger.perReviewRatings` keys on `reviewId`, not a customer identity, and is never returned by any query response (write-side bookkeeping only, per ddd-model.md) — there is no column here analogous to Payment's tokenized-credential case or User's address/phone. Per §24.1.5's own reasoning, the correct classification for a public search-index service is an explicit statement that none applies.
