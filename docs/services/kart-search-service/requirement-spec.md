...
| Observability | Serilog (structured JSON logs) + OpenTelemetry SDK (traces/metrics) → Grafana Loki (logs) / Grafana Tempo (traces); Prometheus scrapes `/metrics`; Grafana dashboards + alerting — per [observability-standards.md](https://github.com/kakon-mehedi/agent-reusables) and [kart-conventions.md](../../standards/kart-conventions.md#observability). STANDARD sampling tier — Search is not one of the four 100%-trace-coverage saga services (`kart-conventions.md` Observability section); traced at the reusable standard's default (100% of error traces, a smaller percentage of successful ones). `GET /search` spans/logs carry a request-scoped `queryId` as the correlation field, since Search's public read (§3 above, §24.1.2) has no per-user or per-entity ownership dimension to key on instead; the indexing-consumer pipeline (`ProductCreated`/`ProductPriceChanged`/`ProductDiscontinued`) carries `sku`, matching the entity those events actually key on. | BRD §23 (stack); `kart-conventions.md` Observability section (Kart-specific tiering) |
