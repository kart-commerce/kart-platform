...
- **Decision (3-5 bullets max):**
  ...
  - Trade-off accepted: the fallback is not actually personalized, so the BRD's own definition of this service ("Personalization") isn't met until a user accumulates signal; this is now fully resolved by `ddd-model.md`'s Recommendation Model — a cold-start user's score degenerates to `trendingBaseline × 0.15`'s relative ordering (no renormalization), so "enough to personalize" is not a separate threshold to tune, it falls out of the formula directly (any non-zero `purchaseAffinity`/`behaviorSignal` term makes an item `Personalized`, per `api-contract.yaml`'s `source` field)
...
- **Why it happens:** requirement-spec §2's Open Question 2 (now resolved — see below) originally noted the BRD names the feature but specifies no algorithm, model, or scoring method — with no defined baseline or target metric at the time this edge case was first drafted, there was nothing stated to measure live output against.
...
- **Decision (3-5 bullets max):**
  ...
  - Why: this is algorithm-agnostic (it measures user reaction to whatever gets recommended) and requires no new infrastructure — Analytics already ingests every platform event, so it is the natural place to compute these metrics rather than Recommendation building its own eval pipeline; an offline held-out-data harness would require a target metric/quality bar the BRD never states, so it remains rejected even now that requirement-spec Open Question 2 (algorithm) is resolved
  - Trade-off accepted: detection is reactive (drift is observed after it has already affected users), not preventive — there is no pre-deployment offline check before a model change ships; accepted because it can be added later without changing anything decided here
  - **Resolved, not carried forward:** requirement-spec Open Question 2 is now closed (`ddd-model.md`'s Recommendation Model — item-based collaborative filtering blended with behavioral/trending signal). The metric this decision measures is click-through on `ProductClicked` events whose `sourceContext` (`event-contract.md`) attributes the click back to a served recommendation, versus the trending-only baseline `source: "Fallback"` items would have gotten — a natural, algorithm-agnostic A/B comparison this service's own event schema already supports. The specific numeric drift threshold remains an operational tuning parameter (not an architectural gap) — genuinely appropriate to set from real traffic data post-launch, not a stakeholder question blocking this design.
