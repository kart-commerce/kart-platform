...
- Consume `ProductCreated` to seed newly catalogued products into the recommendation candidate pool (BRD §10's Event Catalog row; publisher Product; payload `sku, attributes`; 3x retry; `catalog.dlq`). §5.4's condensed row for Recommendation omits this event while §10 lists Recommendation as a consumer — this was a genuine §5.4/§10 inconsistency (the same shape ADR-0005 resolved for `OrderDelivered`/`OrderCompleted`, mirrored), now resolved by ADR-0013 (`docs/adr/0013-recommendation-productcreated-consumption.md`): §10's Event Catalog governs consumer-set questions over §5.4's explicitly "condensed" summary, so Recommendation does consume this event, with the purpose of seeding new products into the fallback/trending pool (see Domain Invariants, API Surface, and `edge-cases.md`'s "Newly catalogued products" edge case, now resolved).
