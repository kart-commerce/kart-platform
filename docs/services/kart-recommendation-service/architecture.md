...
| Inbound (consumed) | Order Service | `OrderDelivered` (Kafka) | **Async** | Purchase-signal input; publisher/payload confirmed by [ADR-0005](../../adr/0005-unify-order-terminal-event.md) (`orderId, deliveredAt`; 3x retry; `order.dlq`). Recommendation depends on Order's output only — Order's own upstream consumption of Delivery Tracking's `DeliveryStatusUpdated` is Order's internal concern, not a Recommendation dependency |
| Inbound (consumed) | Product Service | `ProductCreated` (Kafka) | **Async** | Seeds newly catalogued products into the fallback/trending candidate pool with a neutral default score; consumer relationship confirmed by [ADR-0013](../../adr/0013-recommendation-productcreated-consumption.md) (payload `sku, attributes`; 3x retry; `catalog.dlq`) |
| Inbound (consumed) | Publisher not yet named (frontend/client event emission) | Clickstream event(s) (Kafka) | **Async** | Behavioral-signal input. No event name, schema, payload, publisher, or topic is defined anywhere in the BRD — carried to the **Event Design Agent** (requirement-spec Open Question 1, non-blocking). Topic will follow `kart.<service>.<entity>` (`kart-conventions.md`) once named; partitioned by `userId` per `edge-cases.md`'s "Clickstream volume overwhelms ingestion" decision, with consumer-group autoscaling on partition lag |
...
**2. Staleness/consistency SLA (requirement-spec Open Question 3 — Recommendation is not named in BRD §2.2 the way Search is, so no bound was BRD-forced).** Proposed bound, pending human sign-off alongside the rest of this document:
...
- Clickstream-driven behavioral signal has no committed bound yet — it depends on the event(s) the Event Design Agent defines (Open Question 1) and the algorithm the DDD Agent chooses (Open Question 2); this spec proposes the same 5-minute per-event target as `OrderDelivered` as a placeholder, to be confirmed once both of those are settled.
