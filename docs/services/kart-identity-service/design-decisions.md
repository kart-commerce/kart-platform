...
- **Decision (5 bullets max):**
  - Chosen: Transactional Outbox, publishing to this service's own RabbitMQ topic exchange (`identity.exchange`, per `docs/standards/kart-conventions.md`) — not Kafka — with `UserAccountUpdated`'s payload carrying an added monotonic `updatedAt`/sequence field per edge-cases.md's own decision, so User Service applies last-write-wins by version rather than relying on RabbitMQ delivery order.
...
- **Requirement driving this:** requirement-spec.md's Observability NFR row (§3, enriched) — Serilog + OpenTelemetry per the platform's reusable `observability-standards.md` and this repo's `kart-conventions.md` Observability section; Identity sits on the **STANDARD sampling tier**, not the 100%-trace-coverage tier reserved for the four Order Saga participants (`kart-order-service`/`kart-inventory-service`/`kart-payment-service`/`kart-shipping-service`), but originates the `traceId` most of those downstream traced chains build on (BRD §18 Gateway JWT validation; §24.1 RBAC claim resolution at token-mint time).
...
- **Decision (4 bullets max):**
  ...
  - Why: an ad-hoc per-service tool choice would fragment dashboards/alerting across 18 services and break single-`traceId` correlation — exactly the wrong failure mode for Identity specifically, since its login/refresh calls are the first hop most traced request chains originate from; one shared package/stack keeps that correlation intact platform-wide even though Identity's own spans are sampled at the standard rate, not 100%. Identity's own logs/traces carry `userId` as the correlation field, and the refresh-token reuse-detection path (Domain Invariant §4) is a concrete signal worth its own Grafana alert — a spike in reuse-triggered full-session-family revocations, distinct from ordinary login failures.
  - Trade-off accepted: none material — this is the standards-consistent default, not a genuine trade-off between comparably-viable options (same posture as this doc's Communication Style decision above).
