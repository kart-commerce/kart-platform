...
- **Decision:**
  ...
  - Why: Whether Admin accounts may ever be auto-created from an external assertion alone is a security/governance policy call (how much trust the platform places in the enterprise IdP's own provisioning correctness), not an engineering default this agent can pick — and the two federation paths (Admin vs. customer) plausibly need different answers, compounding the judgment call.
  - Trade-off accepted: None picked — flagged alongside requirement-spec Open Question #7, since both stem from the same unaddressed provisioning gap in BRD §24.2, for human resolution before the Architecture Agent designs the SSO callback flow.
...
- **Decision:**
  - Chosen: Apply an absolute lifetime cap to refresh tokens issued via a federated login, distinct from (and shorter than) whatever cap native sessions eventually get.
  - Why: This closes the specific failure mode — indefinite renewal with no revocation signal ever arriving — without requiring a new IdP integration; it is a defensible engineering default (a bounded cap) rather than the actual fix, which is real-time SLO propagation.
  - Trade-off accepted: A still-employed Admin using a long-lived federated session is forced to re-authenticate against the IdP periodically even though nothing about their access changed — accepted because the alternative (no cap) has unbounded exposure once the IdP has already revoked the person, which conflicts directly with Domain Invariant #2 ("a revoked token must never be accepted again").
  - Note: This mitigates the worst failure mode but does not solve the underlying gap — real-time SLO/deprovisioning propagation remains Open Question #8, unresolved, and is the Architecture Agent's to design once specific IdP capabilities (SCIM, SLO endpoints) are known.
