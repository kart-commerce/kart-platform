...
- Publish `UserNotificationPreferenceUpdated` (`userId`, a per-channel-per-category opt-out map, `appInstalled` boolean) whenever `notificationOptIn` or the app-installed signal changes. **Added here at this closure pass** — a new publish responsibility `kart-notification-service/requirement-spec.md`/`architecture.md` (both approved) already resolved from their own side (their requirement-spec §6 Q3's "how is opt-out data populated" question), naming User Service as the publisher, but not yet reflected in this document until now. This is a translation event from User's own coarse `notificationOptIn` toggle into the finer-grained, per-category shape Notification's own send-time decisions need — User remains the single place a preference is actually set; Notification's copy is a derived, checked-at-send-time projection, never a second source of truth for what the user chose.
...
| `POST /internal/users/{userId}/erasure-requests` | Inbound API | **Added here at this closure pass.** Internal, client-credentials-gated; triggers the ADR-0016 tombstone write. Caller resolved by [ADR-0017](../../adr/0017-user-erasure-request-intake-caller.md): Admin Service, via a fifth business category ("compliance/erasure request intake") added to ADR-0010's enumeration. |
| `UserNotificationPreferenceUpdated` | Published | **Added here at this closure pass.** New publish responsibility resolved from `kart-notification-service`'s own approved docs (see §2 above) — not yet in the BRD's Event Catalog; needs a catalog row added in a later documentation pass, same as `UserAccountUpdated`/`UserDataErased` were. |
