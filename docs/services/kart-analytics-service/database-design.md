...
No `ddd-model.md` exists for this service, matching the same precedent already recorded in `kart-admin-service/database-design.md`: `architecture.md`'s Boundary Rationale fixes Analytics' domain shape as a **Generic Subdomain** with exactly two responsibilities — (1) an idempotent ingestion pipeline landing the full platform event fan-in ([ADR-0004](../../adr/0004-analytics-full-fanin-ingestion.md)) into a raw event store, and (2) a set of read models (dashboards/funnels, requirement-spec.md §6 D4a) computed from that raw store. There is no transactional aggregate root of Analytics' own to model beyond "the raw ingested event" and "the derived read model" — narrow enough to design directly from requirement-spec.md/edge-cases.md/design-decisions.md/architecture.md/api-contract.yaml, the same reasoning `kart-admin-service`'s equivalent doc already used for its own missing `ddd-model.md`.
