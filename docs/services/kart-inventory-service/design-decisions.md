...
- **Requirement driving this:** requirement-spec.md's Observability NFR row (§3, new) — Serilog + OpenTelemetry per the platform's reusable `observability-standards.md` and this repo's `kart-conventions.md` Observability section; Inventory is one of the four **100%-trace-coverage** services (`kart-order-service`/`kart-inventory-service`/`kart-payment-service`/`kart-shipping-service`), since its Reserve/Release stock calls are a synchronous step directly on the Order Saga's critical path (§12.1/§12.2).
...
- **Decision:**
  - Chosen: Serilog (structured JSON logs) + OpenTelemetry SDK (traces/metrics), via `Kart.Shared.Observability`, sampled at 100% (traces and logs) rather than the platform's default partial sampling. Logs export via OTLP → Grafana Loki; traces via OTLP → Grafana Tempo; metrics scraped by Prometheus from `/metrics`; Grafana provides dashboards/alerting.
  - Why: Inventory's reserve/release calls are the first synchronous saga step (§12.1) and the compensating-release step (§12.2) — an oversell bug, a lost lock, or a slow lock-wait timeout is exactly the class of failure that needs a full trace, not a sampled one, to diagnose across the Order→Inventory→Payment→Shipping chain. `sku`/`reservationId` are the correlation fields carried on Inventory's structured logs and trace attributes alongside the mandatory `traceId`/`service`/`level` fields. Concrete signal worth its own dashboard: the lock-wait fail-fast-timeout rate on the `SELECT ... FOR UPDATE` acquisition (Resilience Budget decision above) — a rising rate is the earliest signal of hot-SKU contention, ahead of it becoming a caller-visible reservation failure.
  - Trade-off accepted: 100% sampling raises log/trace volume and ingestion cost on Inventory specifically versus the platform's default partial sampling — accepted because Inventory is the platform's highest-contention service (`PLATFORM_BLUEPRINT.md`) and a mandatory saga participant, the same order-path-criticality trade-off already made for Order/Payment/Shipping.
...
- [x] Chosen technologies/patterns approved as-is
