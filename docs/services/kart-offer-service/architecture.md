...
| Outbound (published) | (none confirmed) | `PriceQuoteIssued` event | **Async** | Consumer unconfirmed — carried to Event/API Design Agent per requirement-spec Q5. Published regardless, as an audit/extension point. |
| Outbound (published) | (none confirmed) | `PromotionActivated` event | **Async** | No listed consumer in BRD. Kept as a domain event for future consumers (e.g., Notification for "sale started" pushes) rather than removed, since the BRD's Notification service consumes "almost every event in catalog" (§5.4). |
