...
| Observability | Serilog (structured JSON logs) + OpenTelemetry SDK (traces/metrics) → Grafana Loki (logs) / Grafana Tempo (traces); Prometheus scrapes `/metrics`; Grafana dashboards + alerting — per [observability-standards.md](https://github.com/kakon-mehedi/agent-reusables) and [kart-conventions.md](../../standards/kart-conventions.md#observability). **STANDARD sampling tier** — Offer is not one of the four 100%-trace-coverage Order Saga services (`kart-order-service`/`kart-inventory-service`/`kart-payment-service`/`kart-shipping-service`); this is the same secondary-path basis already used for this service's Availability tier above (§3). Structured logs/traces/metrics exemplars carry `couponId`/`promotionId` (or `quoteId` for `/pricing/quote` reads) alongside the mandatory `traceId`/`service`/`level` fields. | BRD §23 (stack); `kart-conventions.md` Observability section (Kart-specific tiering); requirement-spec §3 Availability row (secondary-path tier, same basis) |
