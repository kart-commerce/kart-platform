...
- **Why it happens:** requirement-spec Open Question 1 — the BRD's only documented compensation flow unwinds Inventory when Payment fails; it never defines what unwinds when a *later* step fails after Payment already succeeded, so refund-vs-inventory-release ordering is undefined.
...
- **Decision (3-5 bullets max):**
  ...
  - **Status: final, ratified.** This is a single-service Saga-orchestration default (Order is the sole cross-service orchestrator, BRD §5.1 Boundary Rationale) — it changes no other service's contract, so it closes requirement-spec's Open Questions resolution #1 directly, no ADR or further human sign-off needed beyond this pass.
...
- **Why it happens:** requirement-spec Open Question 3 — `POST /orders` has no documented `Idempotency-Key` requirement, unlike Payment's `/payments/charge`, so a plain client-side retry is free to create two orders for one checkout.
...
- **Why it happens:** requirement-spec Open Question 6 (per-step Saga timeout budget) — the BRD itself defines no such budget, and Payment/Shipping communicate asynchronously via events, so nothing prevents a race between a fired timeout and an in-flight downstream success. Now that requirement-spec resolution #6 fixes concrete per-step budgets (Inventory 2s, Payment 30s, Shipping 60s), this race has a bounded, known timeout to guard against rather than an open-ended one.
...
- **Decision (3-5 bullets max):**
  ...
  - **Status: final.** The timeout values that bound this race are now fixed by requirement-spec resolution #6 (Inventory 2s, Payment 30s, Shipping 60s) — a "late" success is any downstream completion event arriving after the relevant per-step budget already elapsed and compensation was triggered. No further open dependency.
...
- **Why it happens:** requirement-spec Open Question 6 (per-step timeout budget) combined with the BRD's own acknowledgment that this is a live concern (§27's interview question "How do you detect a saga that's stuck halfway forever?") without the Saga Pattern section (§12) documenting any stuck-detection mechanism.
...
- **Decision (3-5 bullets max):**
  ...
  - Trade-off accepted: a reconciliation sweep running every 60s does not detect a stuck saga instantly — worst case, up to one sweep interval plus the grace threshold below elapses before compensation is forced.
  - **Staleness threshold — resolved, no longer escalated.** Now that requirement-spec resolution #6 fixes per-step budgets (Inventory 2s, Payment 30s, Shipping 60s), the threshold is set to **2× the budget of the step the order is currently awaiting** — i.e. an order stuck awaiting `InventoryReserved` past 4s, awaiting `PaymentCompleted`/`PaymentFailed` past 60s, or awaiting `ShipmentDispatched` past 120s is swept. The 2× multiplier is a deliberate grace margin over the in-process per-step timeout handler (which should have already fired its own reaction by 1×) — the sweep exists to catch cases where that in-process handler itself failed to run (e.g. a crashed worker), not to duplicate it.
  - **Amendment (this pass, post-ADR-0015): "forces compensation" is no longer uniform across all three thresholds.** For the Inventory-await (4s) and Payment-await (60s) cases, the sweep still forces the original reverse-order compensation (release Inventory, mark `Cancelled`) — both are still pre-confirmation. For the Shipping-await case (120s), the sweep instead forces entry into `FulfillmentException` (see "Shipment Creation Permanently Fails After Order Confirmation" below) — **not** automatic refund-and-cancel — since by this point `PaymentCompleted` has already been received and a silent Shipping timeout is exactly as unsafe to auto-reverse as an explicit `ShipmentCreationFailed`. This is the same scope-narrowing requirement-spec's Open Questions resolution #1 amendment applies.
...
- **Decision (3-5 bullets max):**
  ...
  - Trade-off accepted: an order whose `ShipmentDispatched` is genuinely lost (not just late) holds an unresolved `DeliveryStatusUpdated` for the full window below rather than failing fast.
  - **Hold/retry window — resolved, no longer unresolved.** Bounded to **60s**, matching the Shipping asynchronous dispatch budget fixed in requirement-spec resolution #6 — that is the longest legitimate wait for `ShipmentDispatched` to be processed and the order to reach `Shipped`, so a terminal `DeliveryStatusUpdated` held/requeued (via application-level NACK with backoff, independent of the event's own broker-level 3x retry policy per BRD §10's `DeliveryStatusUpdated` row) for longer than that is genuinely racing ahead rather than merely early. If the 60s window elapses without the order reaching `Shipped`, the event is routed to `tracking.dlq` for the same alerting path already defined for that event, rather than requeued indefinitely.
...
## Edge Case: Shipment Creation Permanently Fails After Order Confirmation

- **What happens:** Order consumes Shipping's `ShipmentCreationFailed` for an order already past `OrderConfirmed` (Payment has already captured funds) — every configured carrier option was exhausted without producing a valid label (bad address, no carrier services the destination).
- **Why it happens:** ADR-0015 introduces this event and states plainly that Order's reaction is "Order Service's own subsequent design decision, not one this ADR makes on Order's behalf" — the BRD's only documented Saga compensation flow (§12.2) unwinds a reservation/charge that never completed, and has no defined behavior for a failure that happens *after* the order is already confirmed and money already captured.
- **Solutions available (3):** run the existing pre-confirmation compensation flow anyway (release Inventory, mark `OrderCancelled`) even though payment was already captured, leaving the refund implicit/undefined · introduce a new, distinguishable non-terminal hold state (`FulfillmentException`) and require an explicit manual/ops action to resolve it (retry, or cancel-with-refund) · auto-cancel and auto-refund immediately with no human review.
- **Decision (3-5 bullets max):**
  - Chosen: new `FulfillmentException` hold state; resolution (retry vs. cancel-with-refund) requires an explicit manual/ops trigger, not automation.
  - Why: reusing the pre-confirmation compensation flow would silently leave a captured charge unaccounted for (that flow was never designed to touch Payment); auto-refunding on the first carrier failure would abandon fulfillment attempts a human could still correct (e.g., a bad address), and the BRD gives no basis for a specific auto-retry policy to pick instead.
  - Trade-off accepted: an order in `FulfillmentException` does not resolve itself — it depends on an operational/admin action, unlike every other Saga transition in this spec, which is either client- or event-driven with no human in the loop.
  - **Status: final.** See requirement-spec.md's "Post-Confirmation Fulfillment Exception Handling" (Saga Orchestration) for the full state-machine placement (`Paid → FulfillmentException`, legal exits back to `Paid` or to `Cancelled` gated on an explicit Payment refund call) and Domain Invariants §4 for how this fits the "Legal state transitions only" rule.

## Edge Case: Chargeback Received Against an Order in Any Post-Payment State

- **What happens:** Payment publishes `ChargebackReceived` for an order that has already moved well past the point pre-confirmation compensation could apply — potentially all the way to `Delivered` — since a chargeback is a bank/card-network-initiated dispute with no relationship to Order's own saga timing.
- **Why it happens:** ADR-0012 introduces this event and states Order must "hold or cancel the order using its own existing compensation logic ... but does not call Payment for a refund," without specifying exactly which state(s) this can legally apply against or how it interacts with the existing `Refunded`-only-reachable-from-`Delivered` invariant this spec had previously established (Open Questions resolution #2) — a chargeback can arrive before delivery too, which that invariant as originally written did not anticipate.
- **Solutions available (3):** treat `ChargebackReceived` as illegal/ignorable before `Delivered` and only actionable afterward, matching the invariant as originally written · carve out a narrow, explicit exception allowing a direct `→ Refunded` transition from any state `Paid` onward (including the non-terminal `FulfillmentException` hold) specifically for this event · introduce a brand-new terminal state distinct from both `Cancelled` and `Refunded` for chargeback-driven closures.
- **Decision (3-5 bullets max):**
  - Chosen: explicit, narrow carve-out — `ChargebackReceived` drives a direct transition to `Refunded` from **any** state `Paid` onward — `Paid`, `Shipped`, `Delivered`, or `FulfillmentException` — plus a state-guarded (idempotent no-op if already released/consumed) Inventory release attempt. `FulfillmentException` is explicitly included: an order held on a fulfillment exception has already had `PaymentCompleted`, so a bank-initiated dispute against it is exactly as legitimate as against any other post-`Paid` state, and a bank's own dispute timeline does not pause for Order's internal hold state.
  - Why: the chargeback has already happened externally (the bank reversed the charge) by the time this event arrives — treating it as "illegal" pre-`Delivered` would leave Order's own read model silently wrong about an order's true payment state; a third terminal state would duplicate `Refunded`'s existing meaning ("money was returned to the customer") for no behavioral difference.
  - Trade-off accepted: the "Legal state transitions only" invariant's `Refunded`-from-`Delivered`-only rule is no longer exceptionless — this is the one explicit, ADR-driven carve-out, not a general loosening of the state machine.
  - **Status: final.** No second Payment-initiated refund is ever triggered from this path — the chargeback itself is the money movement; see requirement-spec.md's "Chargeback Reaction" (Saga Orchestration) and the updated `Refunded`-reachability bullet under Domain Invariants §4.
...
- **Why it happens:** the existing "Client Cancel Request Racing an In-Flight Saga" edge case above resolves cancel-vs-Saga races generally by routing cancel through the same state machine as single writer, but that resolution alone doesn't say what the state-guard should decide when the competing transition is `Shipped → Delivered` specifically — this depended on requirement-spec Open Question 2 (state-machine edge transitions), which at the time this edge case was first written had not yet decided whether `Cancelled` is reachable from `Shipped` at all, compounded by an async terminal event that could win or lose the race against the cancel request. Open Question 2 is now resolved (see Decision below).
- **Solutions available (2):** reject `POST /orders/{id}/cancel` outright once the order is in `Shipped` (or once a terminal `DeliveryStatusUpdated` is already being processed), forcing the client into a separate returns/refund flow instead · allow the cancel call to remain legal from `Shipped` and let the state-guard's tie-break decide — e.g. an in-flight terminal `DeliveryStatusUpdated` always wins because physical delivery already happened, cancellation is moot.
- **Decision (3-5 bullets max):**
  - Chosen: option 1 — reject `POST /orders/{id}/cancel` outright once the order is in `Shipped`; the customer must use the separate returns/refund flow instead.
  - Why: **resolved, no longer escalated.** Requirement-spec Open Questions resolution #2 has now decided the underlying product question this edge case was waiting on: `Cancelled` is reachable only pre-`Shipped`. That decision, once made, resolves this race as a direct consequence rather than a separate tie-break to design — cancellation is already illegal from `Shipped` onward regardless of whether a terminal `DeliveryStatusUpdated` happens to be in flight at the same instant, so "terminal delivery always wins" isn't a new policy invented here, it falls out of the state machine's own legal-transition rule.
  - Trade-off accepted: a client whose cancel request and the order's `Shipped` transition are separated by only milliseconds may see a `409` even though, from their perspective, they clicked "cancel" almost immediately after shipment — this is the same trade-off already accepted in the "Client Cancel Request Racing an In-Flight Saga" edge case above (cancel must wait for/respect the saga's current state), applied at the `Shipped` boundary specifically.
