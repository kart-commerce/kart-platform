...
|---|---|---|---|---|---|---|
| `OrderCreated` | `order.order.created` | Payment, Analytics | `orderId`, `userId`, `items`, `total` | 5x exponential, paged on-call | `order.order-created.dlq` | Elevated from BRD §10's original 3x/manual-replay tier (requirement-spec Open Questions resolution #7) — a stuck `OrderCreated` blocks Payment from ever being invoked, at least as severe as a stuck `PaymentCompleted`, and Order is BRD §3's named 99.99%-tier subject |
