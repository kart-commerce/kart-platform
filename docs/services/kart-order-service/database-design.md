...
INSERT INTO order_events (id, order_id, sequence, from_status, to_status, event_type, payload, created_at)
VALUES ($2, $1, (SELECT COALESCE(MAX(sequence), 0) + 1 FROM order_events WHERE order_id = $1),
        $expected_from_status, $to_status, $event_type_or_null, $payload_or_null, now());
COMMIT;
```
