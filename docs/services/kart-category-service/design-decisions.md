...
- **Decision (3 bullets):**
  - Chosen: Transactional Outbox — the `CategoryUpdated` row (one per create/rename/move/deprecate; one coarse row per subtree move) is written in the same transaction as the taxonomy change, then relayed to the `category.exchange` topic exchange (kart-conventions.md) with the 3x/`catalog.dlq` policy already fixed by ADR-0008.
