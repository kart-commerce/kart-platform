...
- **CanRead/CanWrite/CanDelete ownership (BRD §24.1.2 — service-owned, decided here rather than left implicit now that it is a named platform requirement).** `CanWrite` on `TrackingRecord` is never end-user-facing: it is granted only to the two trusted, non-interactive sources named above — the internal `ShipmentDispatched` consumer (aggregate creation) and this service's own webhook-ingestion pipeline once a carrier's `X-Carrier-Signature` HMAC has verified (requirement-spec §5) — checked inline at each ingestion entry point, never a persisted grant table. `CanDelete` is never exposed to any caller; a `TrackingRecord` is only ever superseded (ordinal guard) or left as-is, never explicitly deleted. `CanRead` via `GET /tracking/{id}` is deliberately **not** ownership-scoped — the opaque `trackingId` itself is the access credential (api-contract.yaml: this route is "public, via API Gateway"), the same "presenting the right identifier is the proof" reasoning `kart-identity-service/ddd-model.md`'s `Session` bootstrapping exception uses for its own bearer-secret lookup — because no `user_id`-shaped field exists anywhere on this aggregate to compare against.
- **Audit-actor invariant (BRD §24.3).** Every write to `tracking_records` stamps `createdBy` (the `ShipmentDispatched` consumer's own `system:*` principal id, at creation) and `updatedBy` (the ingesting carrier adapter's own principal id, e.g. `system:carrier-webhook:{carrierId}` or `system:carrier-poll:{carrierId}`, on every accepted status update) — never `NULL`, and never a human/API caller, since nothing on this aggregate's write path originates from one. Because this service has no PostgreSQL `DbContext`, these columns are stamped explicitly by the ingestion pipeline itself via the shared `kart-shared` `ICurrentPrincipalAccessor` abstraction (§24.3), not an EF Core `SaveChanges` interceptor — see database-design.md.
...
- **CanRead/CanWrite/CanDelete ownership (BRD §24.1.2).** `CanWrite` (append) is gated identically to `TrackingRecord` above — only the internal ingestion pipeline (event consumer or HMAC-verified carrier webhook/poll) may append an entry. `CanDelete` is never exposed to any caller, including internally — append-only is a domain invariant of this aggregate, not merely an access-control default. `CanRead` has no dedicated end-user-facing endpoint at all (requirement-spec.md/api-contract.yaml explicitly do not expose history reads); the only readers are this service's own internal ops/triage tooling (edge-cases.md "Unmapped Carrier Status"), which is a platform-operator concern, not an end-user one — no ownership comparison is meaningful here for the same reason it isn't for Identity's `service_principals`.
- **Audit-actor invariant (BRD §24.3).** Every entry stamps `createdBy` with the writing pipeline's own `system:*` principal id (`system:shipment-dispatched-consumer` for the one `SYSTEM`-anchor entry; `system:carrier-webhook:{carrierId}` / `system:carrier-poll:{carrierId}` for `WEBHOOK`/`POLL` entries) — never `NULL`. No `updatedBy`/`updated_at` applies: entries are immutable once written (append-only invariant above), so there is no "most recent update" to attribute. Stamped via the shared `kart-shared` `ICurrentPrincipalAccessor` at the ingestion pipeline's single append call site, not an EF Core interceptor (no PostgreSQL `DbContext` exists here) — see database-design.md.
...
- **CanRead/CanWrite/CanDelete ownership (BRD §24.1.2).** This is a purely internal idempotency ledger with no end-user-facing read or write surface at all — `CanWrite` (insert on first receipt) and the `expiresAt` recompute are both performed exclusively by this service's own ingestion pipeline, gated the same way as `TrackingRecord`'s writes above; `CanRead` has no API surface whatsoever. `CanDelete` is never an explicit caller action — entries are removed only by MongoDB's own TTL reaper (design-decisions.md). No ownership-comparison concept applies, the same carve-out as `kart-identity-service`'s `idp_group_role_mappings`.
- **Audit-actor invariant (BRD §24.3).** `createdBy` stamps the ingesting pipeline's `system:*` principal id at insert; `updatedBy`/`updated_at` are added to cover the `expiresAt` recompute (the one mutation this aggregate's own invariant above already describes), stamped with `system:delivery-tracking-terminal-status-sweep` — the process that re-targets `expiresAt` once a `trackingId` reaches a terminal status. Never `NULL`; stamped via the shared `kart-shared` `ICurrentPrincipalAccessor`, not an EF Core interceptor — see database-design.md.
