...
//   lastUpdatedAt: <ISODate>           // when currentStatus/eta was last (re)computed by an accepted update — also the polling-fallback staleness clock (edge-cases.md "Carrier Webhook Failure or Silence")
// }
...
//   receivedAt: <ISODate>              // this service's own ingestion time — distinct from eventTimestamp because carrier webhook delivery has no ordering guarantee (edge-cases.md "Out-of-Order Carrier Status Updates"); kept for audit/debugging even though it is never the ordering key
// }
