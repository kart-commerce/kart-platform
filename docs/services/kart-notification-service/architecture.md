...
1. **No public API, consumer-only** (BRD §5.4; reaffirmed by `design-decisions.md`'s Communication Style decision). It has no inbound synchronous surface of any kind — everything it does is triggered by consuming an event off one of the producer exchanges it binds to (below); it owns no exchange of its own for inbound traffic.
2. **Broadest single fan-in consumer on its resolved scope** (ADR-0003): all `order.*`/`payment.*` routed events, plus `WishlistPriceAlertTriggered`, `UserRegistered`, `ShipmentDispatched`, and `DeliveryStatusUpdated` — each bound from its own producer's exchange, never one shared exchange (BRD §8). This scope question was the requirement-spec's Q1 (blocking) and is fully closed by ADR-0003 — cited, not re-litigated, here.
...
RabbitMQ topology (per `design-decisions.md`'s Communication Style decision, and BRD §8's per-service-exchange model): seven consumer queues, each bound to its own producer's exchange, never one shared exchange — `order.exchange` (`order.*`), `payment.exchange` (`payment.*`), `wishlist.exchange` (`wishlist.price-alert.triggered`), `identity.exchange` (`identity.user.registered`), `shipping.exchange` (`shipping.shipment.dispatched`), `tracking.exchange` (`tracking.delivery-status.updated`), and `user.exchange` (`user.notification-preference-updated`, new binding needed for the event introduced below). Every queue dead-letters to Notification's own DLX (`notification.dlx`). This is the concrete topology-finalization the requirement-spec (§5) flagged as carried forward to this stage — now closed.
