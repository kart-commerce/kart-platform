...
- **Decision:**
  - Chosen: RabbitMQ topic-exchange consumer only — per §8's per-service-exchange topology (`docs/standards/kart-conventions.md`), Notification declares its own queues bound directly to each producer's own exchange rather than one shared exchange: `order.exchange` (`order.*`), `payment.exchange` (`payment.*`), plus dedicated bindings on `wishlist.exchange` (`WishlistPriceAlertTriggered`), `identity.exchange` (`UserRegistered`), `shipping.exchange` (`ShipmentDispatched`), `tracking.exchange` (`DeliveryStatusUpdated`); no inbound public API of any kind
