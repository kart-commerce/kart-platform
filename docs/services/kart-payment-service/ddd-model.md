...
- **CanRead/CanWrite/CanDelete ownership (BRD §24.1.2 — service-owned, decided here rather than left implicit now that it is a named platform requirement).** Payment is the resource owner of `PaymentIntent`/`Refund`, and the BRD's own §24.1.2 worked table already names this exact service's mechanism directly: **`CanWrite` (refund) for a Support Agent is gated by an inline business rule — a capped refund amount computed against the order's own total, evaluated per-request against data only Payment holds** (the same "data-dependent rule" reasoning `architecture.md`'s Sync/Async Resolution section already records: "Support Agent can refund, but only up to $X," fine-grained-checked here since the API Gateway's coarse RBAC check cannot know the order total). `CanWrite` (charge) is restricted to two triggers, neither of them a direct customer-facing call: the `OrderCreated` consumer (system-initiated) and Order's own Saga-compensation refund call (architecture.md's Compensation-Refund Trigger) — both reuse the identical idempotent code path a Support Agent's manual refund call also uses. `CanRead` on a `PaymentIntent`/`Refund` is available to Order (system, internal lookup) and Support Agent/Admin per their coarse role (§24.1.1), for dispute/support triage. **`CanDelete` is never exposed to any principal** — `payment_intents`/`refunds` are permanent financial records; nothing in this service's domain ever hard-deletes one (confirmed by database-design.md's GDPR-erasure check: no redaction or deletion path exists because no PII is stored here to begin with).
- **Audit-actor invariant (BRD §24.3).** Every mutation to `payment_intents`, `refunds`, `idempotency_keys`, or `gateway_webhook_events` (database-design.md) stamps `created_by`/`updated_by` with the resolved acting principal — a Support Agent's own principal id for a manual `POST /payments/{id}/refund` call, `system:order-saga-payment-consumer` for the `OrderCreated`-triggered charge or Order's Saga-compensation refund call, or `system:payment-gateway-webhook-consumer` for a `payment_intents`/`refunds` state transition driven by ingesting `POST /payments/webhooks/{gateway}` — never `NULL`. These columns are auto-injected by the shared `kart-shared` `Kart.Shared.Auditing` package's `SaveChanges` interceptor (§24.3 — one platform-wide implementation referenced as a NuGet dependency, not built locally by this service; see database-design.md) and are never populated from a client-supplied request value.
