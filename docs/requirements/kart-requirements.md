...
- **Ownership of the mechanism, decentralized like §24.1.2**: each service wires its own interceptor against its own `DbContext`/ORM instance — there is no shared, cross-service component that injects these columns, since each service owns its own database connection and transaction boundary (§6.1). What's centralized here is the *column shape and the non-negotiable rule that injection is automatic*, not a shared runtime component; a service's own `database-design.md` records that its tables carry these four columns the same way it already records partitioning/indexing choices.
