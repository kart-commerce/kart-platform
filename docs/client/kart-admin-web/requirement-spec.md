...
| Design tokens | Reuses `kart-web`'s brand tokens ([`design-tokens.md`](../kart-web/design-tokens.md)) directly — no separate token file | One brand, one design language — an internal tool with a visibly different brand from the storefront it manages would be confusing, not differentiated for a good reason. |
...
- Session timeout is tighter than `kart-web`'s customer session (elevated-privilege sessions expire faster) — exact duration is a security-policy decision for whoever owns that policy, not invented here; flagged in Open Questions.

## 6. Open Questions

1. **Exact idle-session-timeout duration for elevated (`Admin`) sessions** — no BRD figure exists; a security-policy decision, not an engineering default this doc should invent.
2. **Whether `Support Agent` and `Admin` should be two visually/structurally distinct app shells (different nav, different landing page) or one shell with role-gated sections** — assumed one shell, role-gated sections, matching the platform's own "one coarse role model, fine-grained per-service" pattern (BRD §24.1); revisit if the two roles' actual daily workflows prove too different to share a shell comfortably.
3. **Scope growth** — this spec is intentionally light; expand §3 as `kart-admin-service`'s own back-office categories (§6 of its spec) evolve, rather than this doc drifting stale against that service's own source of truth.
