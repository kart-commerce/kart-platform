...
| Refund request | `kart-payment-service` | `POST /payments/{id}/refund` ✅ — `container-diagram.md` marks this "support-agent driven, BRD §22," so a *customer-initiated* return/refund request likely lands as a request/ticket rather than this exact endpoint | REST | Owner-scoped | 🚧 Flagged: confirm with Payment's own `api-contract.yaml` whether a distinct customer-initiated return endpoint exists or whether `kart-web` only ever displays refund status, not initiates it directly — carried to Open Questions below |
...
## Open Questions Carried From This Map

1. **Customer-initiated return/refund request** — flagged in the Checkout & Fulfillment table above. Confirm against `kart-payment-service`'s own `api-contract.yaml` (and possibly `kart-order-service`'s, if returns are modeled as an Order-side flow rather than a direct Payment call) once it names a customer-facing return endpoint, or confirm the intended flow is "request via support" rather than self-service.
2. **Every 🚧 row** resolves automatically once that service's `api-design-agent` stage produces an approved `api-contract.yaml` — this map should be revisited at that point to flip the status and record the actual path/method rather than a description, not left stale.
