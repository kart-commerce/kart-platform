...
| i18n | Runtime-switchable, ICU MessageFormat, RTL-ready from day one even if launch locales are LTR-only | Cheaper to build in from the start than retrofit; BRD itself doesn't state launch locales (§9 Open Questions). |
...
- Order history, order detail, order cancellation (while cancellable per Order's own state machine), returns/refunds initiation.
...
| i18n | Full runtime i18n with RTL support (§2) | `[default]`, this doc's own decision |
...
Governed by `agent-reusables/docs/standards/frontend/design-system-standards.md` in full; not restated here. Kart's own token values, theme list (light/dark/system, single-brand — no white-label requirement identified for Kart itself, see §9), and asset registry are [`design-tokens.md`](design-tokens.md).
...
The BRD is written from a backend/systems-design-curriculum angle (§1.1–1.2) and does not specify client-tier scope in the same depth it specifies backend services — these are genuine gaps, not resolved here, carried forward to `architecture.md`/`design-tokens.md` as explicit assumptions where a call had to be made to keep moving:

1. **Launch locales/currencies are not stated anywhere in the BRD.** §2's i18n architecture decision (runtime-switchable, RTL-ready) is locale-count-agnostic by design, so this doesn't block building the capability — but the actual initial locale list is a product decision for whoever owns go-to-market, not an engineering default this doc can make.
2. **Product comparison and "recently viewed" (§3.1) have no owning backend service named anywhere in the BRD's 18-service list.** Assumed client-local (browser storage) for "recently viewed" and a thin client-side feature (no new backend service) for comparison, pending confirmation — if either needs cross-device persistence, that's new backend scope, not something `kart-web` can invent unilaterally.
3. **Native mobile app is out of scope for this spec** (§1) — the BRD doesn't request one, and the PWA decision in §2 is this doc's own answer to "how does Kart support mobile" absent a stated native-app requirement. If a native app is later commissioned, it consumes the same `kart-api-gateway` contracts `kart-web` does — no new backend scope, but a new client repo outside this doc's scope.
4. **Whether `kart-web` needs 100% of the catalog server-rendered for SEO, or only PLP/PDP/home** is undetermined — assumed PLP/PDP/home/category pages are SSR'd (the pages organic search actually needs to index) and authenticated-only views (account, order history) are client-rendered-only post-hydration, since SEO has no reason to reach them. Flagged for `architecture.md` to confirm against actual SEO requirements once available.
5. **White-label/multi-brand support** — the reusable design-system standard supports it structurally at no extra cost (§7), but nothing in the BRD suggests Kart itself operates multiple brands. Treated as "supported by the token architecture, not a requirement Kart currently exercises" — see `design-tokens.md`.
