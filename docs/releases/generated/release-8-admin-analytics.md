...
Regenerate this file with `scripts/release.sh 8 --force` after editing `docs/releases/releases.json` -- do not hand-edit the Scope section below, hand-edit the checklists.
...
|---|---|---|
| [`kart-admin-service`](../../services/kart-admin-service/) | 22 |  |
| [`kart-analytics-service`](../../services/kart-analytics-service/) | 17 |  |
...
| Tier | Target | Focus |
|---|---|---|
| High | 1,000,000 RPM | Analytics consumes every event published by every other service simultaneously (ADR-0004) -- this is the true full-fan-in stress test. |
...
- [ ] `kart-admin-service`: requirement-spec, edge-cases, architecture, ddd-model, api/db/event contracts, and tickets are all `status: approved`
- [ ] `kart-analytics-service`: requirement-spec, edge-cases, architecture, ddd-model, api/db/event contracts, and tickets are all `status: approved`
...
|---|---|
| Tag | `v0.8.0` |
| Title | Release 8: Admin & Analytics |
| Milestone | `Release 8: Admin & Analytics` |
...
```markdown
## v0.8.0 -- Release 8: Admin & Analytics
...
```bash
gh release create v0.8.0 --title "Release 8: Admin & Analytics" --notes-file <path-to-filled-in-notes.md>
```
