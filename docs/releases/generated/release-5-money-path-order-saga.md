...
# Release 5: Money Path: Checkout, Payment, Order Saga

> **highest-priority gate in the entire plan -- do not compress this release to hit a date**

**Exit milestone:** MVP-complete: a customer can actually buy something, and the order-critical-path is hardened for flash-sale scale.

Regenerate this file with `scripts/release.sh 5 --force` after editing `docs/releases/releases.json` -- do not hand-edit the Scope section below, hand-edit the checklists.
...
|---|---|---|
| [`kart-payment-service`](../../services/kart-payment-service/) | 11 |  |
| [`kart-order-service`](../../services/kart-order-service/) | 20 | the Saga orchestrator |
...
| Tier | Target | Focus |
|---|---|---|
| High -> Extreme | 1,000,000 then 10,000,000+ RPM | Full Order Saga end-to-end (Order -> Inventory reserve -> Payment charge -> confirm) at High tier, then the full BRD §26 chaos suite (kill an Order pod mid-request, saturate RabbitMQ queue depth, introduce PostgreSQL replica lag, drop 20% inter-service packets), then an Extreme-tier flash-sale breaking-point test. |
...
- [ ] `kart-payment-service`: requirement-spec, edge-cases, architecture, ddd-model, api/db/event contracts, and tickets are all `status: approved`
- [ ] `kart-order-service`: requirement-spec, edge-cases, architecture, ddd-model, api/db/event contracts, and tickets are all `status: approved`
...
- [ ] Executed the High -> Extreme tier load test described above and recorded the result in `docs/benchmarks/` (one dated file per service, see `docs/benchmarks/0000-benchmark-template.md`)
...
|---|---|
| Tag | `v0.5.0` |
| Title | Release 5: Money Path: Checkout, Payment, Order Saga |
| Milestone | `Release 5: Money Path: Checkout, Payment, Order Saga` |
...
```markdown
## v0.5.0 -- Release 5: Money Path: Checkout, Payment, Order Saga
...
```bash
gh release create v0.5.0 --title "Release 5: Money Path: Checkout, Payment, Order Saga" --notes-file <path-to-filled-in-notes.md>
```
